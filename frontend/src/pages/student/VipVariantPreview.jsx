...
const VipVariantPreview = ({ sectionType, sectionName, filename, onClose }) => {
  ...
  const loadVariantData = async () => {
    try {
      const response = await studentApi.getVipVariantPreview(sectionType, sectionName, filename);
      setVariantData(response.data);
    } catch (error) {
      showToast('Variantni yuklashda xatolik', 'error');
    } finally {
      setLoading(false);
    }
  };
        ...
        {variantData.questions && variantData.questions.length > 0 && (
          <div>
            <h4 className="font-medium text-gray-900 dark:text-white mb-3">
              Savollar ({variantData.questions.length}):
            </h4>
  ...
  const renderReadingPreview = () => {
    ...
    return (
      <div className="space-y-4">
        ...
        {variantData.text && (
          <div>
            <h4 className="font-medium text-gray-900 dark:text-white mb-3">Matn:</h4>
  ...
  const renderSpeakingPreview = () => {
    ...
    return (
      <div className="space-y-4">
        ...
        {variantData.topic && (
          <div>
            <h4 className="font-medium text-gray-900 dark:text-white mb-2">Mavzu:</h4>
            <p className="text-sm text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-800 p-3 rounded">
              {variantData.topic}
            </p>
          </div>
        ...
        {variantData.questions && variantData.questions.length > 0 && (
          <div>
            <h4 className="font-medium text-gray-900 dark:text-white mb-3">Savollar:</h4>
            <div className="space-y-2">
              {variantData.questions.map((q, idx) => (
                <div key={idx} className="p-3 bg-gray-50 dark:bg-gray-800 rounded">
...
export default VipVariantPreview;

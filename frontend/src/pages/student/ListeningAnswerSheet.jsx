...
const ListeningAnswerSheet = () => {
  ...
  const handleAnswerChange = (questionId, value) => {
    updateAnswer('listening', questionId, value.toUpperCase());
    studentApi.saveListeningAnswers({ ...answers.listening, [questionId]: value.toUpperCase() }).catch(console.error);
  };

  const handleTimeout = () => {
    navigate(`/exam/${key}/writing`);
  };
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 py-4 flex items-center justify-between mb-6 rounded-lg shadow-sm">
        <h1 className="text-xl font-semibold text-gray-900 dark:text-white">Listening Answer Sheet</h1>
      ...
      <div className="flex justify-end mt-6">
        <Button onClick={() => navigate(`/exam/${key}/writing`)}>
          Continue to Writing
        </Button>
      </div>
    </div>
